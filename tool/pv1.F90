...
!    # Full speed run:
!    ifort -O2 pv1.F90 -o pv1.exe -lnetcdf -lnetcdff -lhdf5 -lhdf5_hl -lz -limf -lm
...
#include <netcdf.inc>
        ...
        real, dimension (:,:), allocatable :: VB, THB, THT, A
...
!c  *********************************
        ...
        real :: plev(nmax) = -9999.9 
        !data pr/ 1.0, 0.925, 0.85, 0.7, 0.5, 0.4, 0.3, 0.25, 0.2, 0.15, 0.1, 0.07/
        !data plev /1000.,925.,850.,700.,500.,400.,300.,250.,200.,150.,100.,70./

        integer :: h_fid, t_fid, u_fid, v_fid
        integer :: xx, yy, lev, is ,ie, js, je
        real, dimension (:), allocatable :: lon_in, lat_in, plev_in, time_in
        real, dimension (:), allocatable :: lon_out, lat_out
        real, dimension (:,:,:,:), allocatable :: h_in, t_in, u_in, v_in
        real, dimension (:,:,:,:), allocatable :: h_in_s, t_in_s, u_in_s, v_in_s
        real, dimension (:,:,:,:), allocatable :: h_in_p, t_in_p, u_in_p, v_in_p
        ...
        ! IFS variables 
        character*300 :: h_infile, t_infile, u_infile, v_infile
        character*300 :: outfile1, outfile2
        character*90 :: x_name, y_name, level_name, time_name, h_name, &
                        t_name, u_name, v_name, time_unit
    
        namelist /nlist/ outfile1, outfile2, h_infile, t_infile, u_infile, v_infile, &
                         dx, dy, west, east, south, north, imax, omegs, thrs, plev, &
                         x_name, y_name, level_name, time_name, &
                         h_name, t_name, u_name, v_name, geopotential
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        ny = int(( north - south )/dy) + 1

        print *, 'Range = ', west, east, south, north
        print *, 'grid numbers (nx, ny) in the ranage = ', nx, ny
        ...
        print *, 'number of levels to be used = ', nw
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        call open_ncfile(h_infile,h_fid)
        call open_ncfile(t_infile,t_fid)
        call open_ncfile(u_infile,u_fid)
        call open_ncfile(v_infile,v_fid)
        print *, 'Reading h from ', trim(h_infile), h_fid
        print *, 'Reading t from ', trim(t_infile), t_fid
        print *, 'Reading u from ', trim(u_infile), u_fid
        print *, 'Reading v from ', trim(v_infile), v_fid

        call get_ncdim1(h_fid, level_name, lev)
        call get_ncdim1(h_fid, x_name, xx)
        call get_ncdim1(h_fid, y_name, yy)
        call get_ncdim1(h_fid, time_name, nt)
        print *, 'data nlon, nlat, np, nt = ', xx, yy, lev, nt

        allocate(plev_in(lev), lon_in(xx), lat_in(yy), time_in(nt))
        call get_var1_real (h_fid, level_name, lev, plev_in)
        call get_var1_real (h_fid, x_name, xx, lon_in)
        call get_var1_real (h_fid, y_name, yy, lat_in)
        call get_var1_real (h_fid, time_name, nt, time_in)


        print *, plev_in
        time_unit = 'not found'
        call get_att_text(h_fid, time_name, 'units', time_unit)
        print *, trim(time_unit), len(trim(time_unit))

        allocate(h_in(xx,yy,lev,nt))
        allocate(t_in(xx,yy,lev,nt))
        allocate(u_in(xx,yy,lev,nt))
        allocate(v_in(xx,yy,lev,nt))
        call get_var4_real (h_fid, h_name, xx, yy, lev, nt, h_in)        
        call get_var4_real (t_fid, t_name, xx, yy, lev, nt, t_in)
        call get_var4_real (u_fid, u_name, xx, yy, lev, nt, u_in)
        call get_var4_real (v_fid, v_name, xx, yy, lev, nt, v_in)

        if ( geopotential ) h_in(:,:,:,:) = h_in(:,:,:,:)/gg

        do i = 1, xx-1
          if (lon_in(i) <= west .and. lon_in(i+1) > west ) is = i
        enddo
        do j = 1, yy-1
          if (lat_in(j) <= south .and. lat_in(j+1) > south ) js = j
        enddo
        ie = is + nx - 1
        je = js + ny - 1
 
        print *, 'is, ie, js, je = ', is, ie, js, je 
        print *, 'True range: ', lon_in(is), lon_in(ie), lat_in(js), lat_in(je)

        allocate(lon_out(nx),lat_out(ny))
        do i = 1, nx
          lon_out(i) = lon_in(i+is-1)
        enddo
        do j = 1, ny
          lat_out(j) = lat_in(j+js-1)
        enddo
 
        !! only use selected levels
        allocate (h_in_p(xx,yy,nw,nt), t_in_p(xx,yy,nw,nt))
        allocate (u_in_p(xx,yy,nw,nt), v_in_p(xx,yy,nw,nt))
        do n =1, nt
          do k = 1, nw
            do p = 1, lev
              if ( plev(k) == plev_in(p) ) then
                 h_in_p(:,:,k,n)  = h_in(:,:,p,n)
                 t_in_p(:,:,k,n)  = t_in(:,:,p,n)
                 u_in_p(:,:,k,n)  = u_in(:,:,p,n)
                 v_in_p(:,:,k,n)  = v_in(:,:,p,n)
          ...
          enddo
        enddo
        deallocate(h_in, t_in, u_in, v_in)

        !! Apply the nine point local smoothing, follow by Grads smth9 function
        !  The result at each grid point is a weighted average of the grid point plus the 8 surrounding points. 
        !  The center point receives a wieght of 1.0, the points at each side and above and below receive 
        !  a weight of 0.5, and corner points receive a weight of 0.3
        !! Also, cut the inversion resion out from the global data
        allocate (h_in_s(nx,ny,nw,nt), t_in_s(nx,ny,nw,nt))
        allocate (u_in_s(nx,ny,nw,nt), v_in_s(nx,ny,nw,nt))
        do n = 1, nt
        do p = 1, nw
           do i = is, ie
           do j = js, je

              h_in_s(i-is+1,j-js+1,p,n) = h_in_p(i,j,p,n) + \
               (0.5/4.)*( h_in_p(i-1,j,p,n) + h_in_p(i+1,j,p,n) + h_in_p(i,j-1,p,n)+h_in_p(i,j+1,p,n)    \
                         -4.*h_in_p(i,j,p,n)) + \
               (0.3/4.)*(h_in_p(i-1,j+1,p,n)+h_in_p(i+1,j+1,p,n)+h_in_p(i-1,j-1,p,n)+h_in_p(i+1,j-1,p,n) \
                         -4.*h_in_p(i,j,p,n))

              t_in_s(i-is+1,j-js+1,p,n) = t_in_p(i,j,p,n) + \
               (0.5/4.)*( t_in_p(i-1,j,p,n) + t_in_p(i+1,j,p,n) + t_in_p(i,j-1,p,n)+t_in_p(i,j+1,p,n)    \
                         -4.*t_in_p(i,j,p,n)) + \
               (0.3/4.)*(t_in_p(i-1,j+1,p,n)+t_in_p(i+1,j+1,p,n)+t_in_p(i-1,j-1,p,n)+t_in_p(i+1,j-1,p,n) \
                         -4.*t_in_p(i,j,p,n))

              u_in_s(i-is+1,j-js+1,p,n) = u_in_p(i,j,p,n) + \
               (0.5/4.)*( u_in_p(i-1,j,p,n) + u_in_p(i+1,j,p,n) + u_in_p(i,j-1,p,n)+u_in_p(i,j+1,p,n)    \
                         -4.*u_in_p(i,j,p,n)) + \
               (0.3/4.)*(u_in_p(i-1,j+1,p,n)+u_in_p(i+1,j+1,p,n)+u_in_p(i-1,j-1,p,n)+u_in_p(i+1,j-1,p,n) \
                         -4.*u_in_p(i,j,p,n))

              v_in_s(i-is+1,j-js+1,p,n) = v_in_p(i,j,p,n) + \
               (0.5/4.)*( v_in_p(i-1,j,p,n) + v_in_p(i+1,j,p,n) + v_in_p(i,j-1,p,n)+v_in_p(i,j+1,p,n)    \
                         -4.*v_in_p(i,j,p,n)) + \
               (0.3/4.)*(v_in_p(i-1,j+1,p,n)+v_in_p(i+1,j+1,p,n)+v_in_p(i-1,j-1,p,n)+v_in_p(i+1,j-1,p,n) \
                         -4.*v_in_p(i,j,p,n))
        ...
        enddo
        enddo
        deallocate (h_in_p, t_in_p, u_in_p, v_in_p)

        ! wrtie out h_in_s, t_in_s, u_in_s, v_in_s in the cutting range
        print *, 'outfile1 = ', trim(outfile1)
        ...
        if (trim(time_unit)/= 'not found') then 
          status = nf_put_att_text(id, timeid, 'units', len(trim(time_unit)),trim(time_unit) )
        else
          status = nf_put_att_text(id, timeid, 'units', 30,'days since 1970-01-01 00:00:00' )
        endif
        ...
        start(1:1)=(/1/)
        total(1:1)=(/nx/)
        status = nf_put_vara_real(id, lonid, start, total, lon_out)
        start(1:1)=(/1/)
        total(1:1)=(/ny/)
        status = nf_put_vara_real(id, latid, start, total, lat_out)
        start(1:1)=(/1/)
        total(1:1)=(/nw/)
        status = nf_put_vara_real(id, levid, start, total, plev)
        start(1:1)=(/1/)
        total(1:1)=(/nt/)
        ...
        status = nf_put_vara_real(id, hid, start, total, h_in_s)
        status = nf_put_vara_real(id, tid, start, total, t_in_s)
        status = nf_put_vara_real(id, uid, start, total, u_in_s)
        status = nf_put_vara_real(id, vid, start, total, v_in_s)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        print *, 'True range: ', lon_in(is), lon_in(ie), lat_in(js), lat_in(je)
        hdr(1) = lat_in(js) ! The range 1: staring y, 2. starting x, 
        hdr(2) = lon_in(is) !           3: ending y,  4. ending x
        hdr(3) = lat_in(je) ! JHC: The original comments in the code are wrong!
        hdr(4) = lon_in(ie)        
        hdr(5) = dx    ! data horizonatl resolution 
        hdr(6) = dy      
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do 108 ll=1,nt
         ...
         do k = 1, nw
            do j = ny,1,-1
               do i = 1, nx
                  h(j,i,k)  = h_in_s(i,ny-j+1,k,ll)
                  th(j,i,k) = t_in_s(i,ny-j+1,k,ll)
                  u(j,i,k)  = u_in_s(i,ny-j+1,k,ll)
                  v(j,i,k)  = v_in_s(i,ny-j+1,k,ll)
               enddo
             enddo
         enddo
